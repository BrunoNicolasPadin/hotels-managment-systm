    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div>
                    ...
                    <div id="dropdownAction" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownActionButton">
                            <li>
                                <a href="{{ route('lovs.create') }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Create</a>
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            @if ($lovs->count() > 0)
                ...
                <form class="my-6" action="{{ route('lovs.index') }}">
                    <div class="flex">
                        <select name="filter" class="text-sm font-medium text-center text-gray-900 bg-gray-100 border border-gray-300 rounded-l-lg hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100">
                            ...
                            <option value="code" @if (isset($params['filter']) && $params['filter'] === 'code') selected @endif>Code</option>
                            ...
                            @foreach ($lovs as $lov)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="grid grid-cols-2 px-6 py-4 space-x-3">
                                        <a href="{{ route('lovs.edit', $lov->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                        <form method="POST" action="{{ route('lovs.destroy', $lov->id) }}">
                                            @method('delete')
                                            @csrf
                                            <button type="submit"
                                                onclick="return confirm('¿Are you sure do you want to remove {{ $lov->name }}?')"
                ...
                <div class="my-4">
                    {{ $lovs->appends($params)->links() }}
                </div>
            @else
                <div class="text-center mt-3">
                    <h1 class="text-3xl">List of values not found</h1>
                </div>
            @endif
        </div>
    </div>
